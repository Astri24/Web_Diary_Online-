  ...
  <div class="section-body">
    <div class="row mt-sm-4">
        <div class="col-12 col-md-12 col-lg-12">
            <div class="card">
                <form method="post" action="{{ route('schedule-back.update', $schedule_subject->id) }}" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="card-body">
                        <div class="row">
                            <div class="form-group col-md-6 col-12">
                                <label>Hari</label>
                                <select class="form-control" name="edit_day_id">
                                  @foreach($schedule_day_origin as $schedule_days)
                                    <option value="{{ $schedule_days->id }}" {{ $schedule_days->id == $schedule_subject->days->id ? 'selected' : '' }}>{{ $schedule_days->hari }}</option>
                                  @endforeach
                                </select>
                              ...
                              <div class="form-group col-md-6 col-12">
                                  <label>Matkul</label>
                                  <input type="text" class="form-control @error('edit_matkul') is-invalid @enderror" name="edit_matkul" value="{{ old('edit_matkul', $schedule_subject->matkul) }}" placeholder="Matkul"> 
                                  @error('edit_matkul')
                                    <div class="mt-1">
                              ...
                              <div class="form-group col-md-6 col-12">
                                  <label>Waktu</label>
                                  <p>Dari</p>
                                  @foreach(explode(',',$schedule_subject->waktu) as $key => $item)
                                    @if($key == 1)
                                      @break
                                    @endif
                                    <input type="time" class="form-control @error('edit_waktu') is-invalid @enderror" name="edit_waktu[]" value="{{ old('edit_waktu', $item) }}" placeholder="Dari"> 
                                  @endforeach
                                  @error('edit_waktu')
                                    <div class="mt-1">
                                        <span class="text-danger">{{ $message }}</span>
                              ...
                              <div class="form-group col-md-6 col-12 d-flex flex-column justify-content-end">
                                  ...
                                  @foreach(explode(',', $schedule_subject->waktu) as $key => $item)
                                    ...
                                    <input type="time" class="form-control @error('edit_waktu') is-invalid @enderror" name="edit_waktu[]" value="{{ old('edit_waktu', $item) }}" placeholder="Sampai"> 
                                  @endforeach
                                  @error('edit_waktu')
                                    <div class="mt-1">
                                        <span class="text-danger">{{ $message }}</span>
      ...
      <div class="modal-header">
        ...
        <div class="modal-body">
          <button type="button" data-toggle="modal" data-target="#tambahHari" id="tambahModalButton" class="btn btn-sm btn-primary mb-3"><i class="fas fa-plus-circle"></i></button>
            @foreach ($schedule_day_origin as $item)
                <div class="card">
                  <div class="card-body">
